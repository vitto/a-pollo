...
var getWidgetWorkingDays = function(widgets) {
    var i, j, widgetDevelopmentDays, widgetDate, latestDate;
    latestDate = '';
    widgetDevelopmentDays = 0;
    for (i = 0; i < widgets.length; i += 1) {
        for (j = 0; j < widgets[i].length; j += 1) {
            if (widgets[i][j].date !== undefined) {
                widgetDate = moment(widgets[i][j].date).format('YYYY-MM-DD');
                if (latestDate !== widgetDate) {
                    latestDate = widgetDate;
                    widgetDevelopmentDays = widgetDevelopmentDays + 1;
                }
            }
        }
    }
    return widgetDevelopmentDays;
};
