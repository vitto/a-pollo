...
var getSnippetsTotal = function(snippets) {
  var snippetsTotal = 0;
  for (var i = 0; i < snippets.length; i += 1) {
    if (snippets[i][0].isSnippet) {
      snippetsTotal += 1;
    }
  }
...
exports.setProjectStats = function(config, snippets) {
    ...
    var lastSnippetDate, dateDiff, dateDuration,
        ...
        snippetWorkingDays, snippetsTotal, snippetExamplesCount, snippetExamplesTotal, snippetsTotalNum;

    lastSnippetDate = getLastSnippetDate(snippets);
    snippetWorkingDays = getSnippetWorkingDays(snippets);
    snippetsTotalNum = getSnippetsTotal(snippets);
    ...
    snippetsTotal = numberPlurals(snippetsTotalNum, 'snippet', 'snippets');
    ...
    config.stats = {
        date : {
            ...
            endValue: moment(lastSnippetDate).format(dateValueFormat),
...
exports.toHexo = function(data) {
    ...
    var post = {
        ...
        dateValue: moment(data[data.length-1].date).format(dateValueFormat),
    ...
    if (data[0].isDoc) {
        ...
        post.annotations = data;
        for (var i = 0; i < post.annotations.length; i += 1) {
          if (Array.isArray(post.annotations[i].date)) {
            post.annotations[i].latestDate = post.annotations[i].date[post.annotations[i].date.length-1];
          } else {
            post.annotations[i].latestDate = post.annotations[i].date;
          }
        }
    }
