...
function saveMap (data, docs, buildPath, cb) {
  var pages = []
  ...
  const filePath = assetsPath(buildPath)
  ...
  const mapPath = path.join(buildPath, 'map.yml')
  ...
  rimraf(config.build, function (err) {
    ...
    saveDocs(config, docs, function (err) {
      ...
      saveAssets(config, data, function (err) {
        if (err) throw err
        saveMap(data, docs, config.build, function (err, map) {
          if (err) throw err
