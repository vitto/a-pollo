...
function saveDocs (config, docs, cb) {
  async.eachLimit(docs, 10,
    function (doc, done) {
      ...
      var frontMatterAnnotations = { annotations: doc.annotations }

      if (config.front_matter) {
        const a = frontMatterAnnotations.annotations[0]
        if (a.annotation !== 'color' && a.annotation !== 'typography') {
          if (a.name) { frontMatterAnnotations.title = a.name }
        } else {
          frontMatterAnnotations.title = a.annotation.charAt(0).toUpperCase() + a.annotation.slice(1)
        }
        if (a.category) { frontMatterAnnotations.category = a.category }
        // if (a.annotation) { frontMatterAnnotations.layout = `${a.annotation}.twig` }
