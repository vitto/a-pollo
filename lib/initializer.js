...
var saveConfig = function(result) {
    var config = {
        ...
        url: result.url,
        pages: path.inside(result.pages),
        ...
        style: {
            ...
            images: path.inside(result['style.images']),
            fonts: path.inside(result['style.fonts'])
        }
    };
    ...
    prompt.get([
        {
            ...
            name: 'style.docs',
            description: 'style.docs'.yellow + ' (Path to documented files you will write, SASS, LESS or CSS supported)'.red,
            required: true,
